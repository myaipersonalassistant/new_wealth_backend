 ...
 * Recipients from: email_subscriptions, foundation_orders, seminar_orders, book_orders,
 * user_profiles, course_enrollments (aligned with frontend adminFirestore)
 */
...
export async function getCampaignRecipients(recipientFilter, withNames = false) {
  ...
  for (const doc of subsSnap.docs) {
    ...
    if (recipientFilter && recipientFilter.startsWith('source:')) {
  ...
  // 2–6. All other email sources (aligned with frontend adminFirestore fetchAllEmailRecords)
  // Only skip when filtering by subscription source — those segments use subscription docs only above.
  if (!(recipientFilter && recipientFilter.startsWith('source:'))) {
    const addEmail = (email, firstName) => {
      const e = (email || '').trim().toLowerCase();
      if (!e || seen.has(e)) return;
      seen.add(e);
      if (withNames) result.push({ email: e, firstName: firstName || e.split('@')[0] });
    };

    const foundationSnap = await db.collection('foundation_orders').get();
    for (const doc of foundationSnap.docs) {
      const d = doc.data();
      addEmail(d.customer_email || d.email, d.customer_name || d.name);
    }

    try {
      const seminarSnap = await db.collection('seminar_orders').get();
    ...
    try {
      const bookSnap = await db.collection('book_orders').get();
    ...
    for (const doc of profilesSnap.docs) {
      const d = doc.data();
      addEmail(d.email, d.full_name?.split(' ')[0]);
    }
